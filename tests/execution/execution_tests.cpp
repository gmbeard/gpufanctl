...
#define EXEC_STRINGIFY_IMPL(x) #x
#define EXEC_STRINGIFY(x) EXEC_STRINGIFY_IMPL(x)
#define EXEC_CHECK(cond)                                                       \
    ::gfc::execution::check((cond),                                            \
                            EXEC_STRINGIFY(cond) "\n  at " __FILE__            \
                                                 ":" EXEC_STRINGIFY(__LINE__))
...
auto check(bool condition, std::string_view msg) noexcept -> void
{
    if (condition) [[likely]]
        return;
...
struct Box
{
    ...
    {
        EXEC_CHECK(std::is_trivially_destructible_v<T> || !constructed_);
...
struct StopWhenOperation
{
    ...
    auto set_done()
    {
        ...
        if (n == 1) {
            if (error) {
                static_cast<Receiver&&>(receiver).set_error(std::move(error));
            }
            else {
                static_cast<Receiver&&>(receiver).set_done();
            }
